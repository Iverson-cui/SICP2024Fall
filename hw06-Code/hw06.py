...
class shiftcipher(methods):
    ...
    def encrypt(self, message):
        ...
        for char in message:
            if char.isalpha():
                # If the character is a letter, we shift it.
                # some character may be shifted out of alphabet scope, so we need base and %26
                base = ord("a") if char.islower() else ord("A")
                # shift_amount = self.shift
                new_char = chr((ord(char) - base + self.shift) % 26 + base)
                result += new_char
            else:
                # If the character is not a letter, we do not change it.
                result += char
        return result
...
class fencecipher(methods):
    ...
    def encrypt(self, message):
        ...
        rails_list = [[] for _ in range(self.rails)]
        index = 0
        direction = 1
        for char in message:
            rails_list[index % self.rails].append(char)
            index += direction
        return "".join(["".join(rail) for rail in rails_list])
...
class splitencryption(extensions):
    ...
    def decorator(self, function, message):
        ...
        after_extract = message[0]  # Keep the first character
        extract = ""
        for i in range(1, len(message)):
            if i % self.split_index == 0:
                extract += message[i]
            else:
                after_extract += message[i]
        message = after_extract + extract
...
class Player:
    def __init__(self, deck, name):
        ...
        self.deck = deck
        self.name = name
        # hand is a list containing the player's cards.
        self.hand = []
        # Draw 5 cards from the deck to start with.
        for _ in range(5):
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        for _ in range(3):
            opponent.hand.pop(0)  # Discard the first 3 cards in opponent's hand
        for _ in range(3):
            opponent.draw()  # Draw 3 new cards from the opponent's deck
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        # Add the opponent's card's attack and defense to all cards in the player's deck.
        for card in player.deck.cards:
            card.attack += other_card.attack
            card.defense += other_card.defense
        # Remove all cards in the opponent's deck that share an attack or defense stat with the opponent's card.
        opponent.deck.cards = [
            card
...
class Fib:
    ...
    def next(self):
        if self.value == 0:
            ...
            next = Fib(self.value + self.previous.value)
            next.previous = self
            return next
